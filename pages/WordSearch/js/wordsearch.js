(function(){
  ...
  function WordSearch(wrapEl, settings) {
    ...
    /*var default_settings = {
      ...
      'gridSize': 12,
      'words': ["sunday","monday","tuesday","wednesday","thusday","friday","saturday"],
	  'wordsList' : [],
      'debug': false
    }
    ...
    // Function to get a random word list
    ...
    var default_settings = {
      'directions': ['W', 'N', 'WN', 'EN'],
      'gridSize': 12,
      'words': getRandomWordList(), // Set the random word list
      'wordsList': [],
      'debug': false
    };
    
    // Merge settings (example)
  ...
  const colorPalette = ['#FF5733', '#33FF57', '#3357FF', '#F1C40F', '#9B59B6', '#1ABC9C', '#E74C3C', '#3498DB'];
  ...
  WordSearch.prototype.lookup = function(selected) {
    ...
    if (this.settings.words.indexOf(words[0]) > -1 ||
      ...
      // Generate a single random color for the current selection
      const selectionColor = getRandomColorFromPalette();
      ...
      for (var i = 0; i < selected.length; i++) {
        ...
        el.style.backgroundColor = selectionColor;  // Use the same color for all selected elements
